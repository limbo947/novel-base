...
try:
    from bootstrap_story_analysis import (
        ...
        NARRATIVE_STAGE_RATIOS,
        NARRATIVE_STAGE_NAMES_AFTER_OPENING,
        OPENING_FIXED_CHAPTERS,
        MIN_CHAPTERS_FOR_NARRATIVE,
...
def build_stages(chunks: list[Chunk], stage_size: int) -> list[Stage]:
    """按固定 chunk 数量构建阶段（fixed 模式回退）"""
    if not chunks:
        return []
    return [
        Stage(
            stage_id=index // stage_size + 1,
            stage_name=f"阶段{index // stage_size + 1}",
            chunk_ids=[chunk.chunk_id for chunk in chunks[index : index + stage_size]],
        )
        for index in range(0, len(chunks), stage_size)
    ]


def build_stages_by_narrative(chunks: list[Chunk]) -> list[Stage]:
    """按叙事阶段构建 Stage 列表"""
    if not chunks:
        return []

    stage_map: dict[str, list[str]] = {name: [] for name in NARRATIVE_STAGE_NAMES}

    for chunk in chunks:
        stage_name = chunk.narrative_stage or _infer_stage_from_position(chunk, chunks)
        if stage_name in stage_map:
            stage_map[stage_name].append(chunk.chunk_id)

    stages = []
    stage_id = 0
    for name in NARRATIVE_STAGE_NAMES:
        chunk_ids = stage_map[name]
        if chunk_ids:
            stage_id += 1
            stages.append(Stage(stage_id, name, chunk_ids, ""))

    return stages


def _infer_stage_from_position(chunk: Chunk, all_chunks: list[Chunk]) -> str:
    """当 chunk 没有叙事阶段标签时，按位置推断"""
    try:
        idx = all_chunks.index(chunk)
    except ValueError:
        return "中期"

    ratio = idx / len(all_chunks)
    opening_ratio = OPENING_FIXED_CHAPTERS / max(100, len(all_chunks) * 1.25)
    cumulative = opening_ratio

    if ratio < cumulative:
        return "开篇"

    remaining_ratio = 1.0 - opening_ratio
    for ratio_frac, name in zip(NARRATIVE_STAGE_RATIOS, NARRATIVE_STAGE_NAMES_AFTER_OPENING):
        cumulative += remaining_ratio * ratio_frac
        if ratio < cumulative:
            return name
...
def summarize_project(project_dir: Path) -> dict:
    ...
    stage_mode = manifest.get("stage_mode", "fixed")
    stages = build_stages(chunks, stage_size) if stage_mode == "fixed" else build_stages_by_narrative(chunks)
    ...
    required_files = list(REQUIRED_SUPPORT_FILES) + [f"outputs/方法卡/{name}.md" for name in METHOD_CARD_NAMES]
    missing_support_files = [relative for relative in required_files if not (project_dir / relative).exists()]

    current_stage = None
    if progress.get("current_stage"):
        current_stage = progress["current_stage"]
    elif pending_chunk_ids:
        stage = next((s for s in stages if pending_chunk_ids[0] in s.chunk_ids), None)
        if stage:
            current_stage = {
...
def print_resume_summary(summary: dict) -> None:
    ...
    current_stage = summary.get("current_stage")
    if current_stage:
        stage_label = current_stage.get("stage_name", "")
        stage_id = current_stage["stage_id"]
        chunk_range = current_stage["chunk_range"]
        if stage_label and stage_label not in ("阶段1", "阶段2", "阶段3", "阶段4", "阶段5"):
            print(f"当前阶段: {stage_label} (阶段 {stage_id}, {chunk_range})")
        else:
            print(f"当前阶段: 阶段 {stage_id} ({chunk_range})")
...
def perform_deep_quality_checks(project_dir: Path, summary: dict) -> dict:
    """执行深度质量检查，包括逻辑错误、格式错误、语法错误检查，以及覆盖度、多样性、完整性检查"""
    issues: list[str] = []
    warnings: list[str] = []
    suggestions: list[str] = []
    
    # 1. 检查最终产物的逻辑错误、格式错误、语法错误
    issues.extend(check_final_output_errors(project_dir, summary))
    
    # 2. 检查覆盖度（案例卡是否覆盖小说前期、中期、后期）
    coverage_warnings, coverage_suggestions = check_case_coverage(project_dir, summary)
    warnings.extend(coverage_warnings)
    suggestions.extend(coverage_suggestions)
    
    # 3. 检查多样性（方法卡和案例卡类型的多样性）
    diversity_warnings, diversity_suggestions = check_output_diversity(project_dir, summary)
    warnings.extend(diversity_warnings)
    suggestions.extend(diversity_suggestions)
    
    # 4. 检查完整性（书籍画像、方法卡、案例卡的完整性）
    completeness_warnings, completeness_suggestions = check_output_completeness(project_dir, summary)
...
def check_final_output_errors(project_dir: Path, summary: dict) -> list[str]:
    """检查最终产物中的逻辑错误、格式错误、语法错误"""
    errors = []

    book_profile_path = project_dir / "outputs" / "书籍画像.md"
    if book_profile_path.exists():
        content = read_utf8(book_profile_path)

        worth_learning_match = re.search(r"最值得学的地方[：:]\s*(.+)", content)
        not_suitable_match = re.search(r"不适合直接参考的部分[：:]\s*(.+)", content)
        if worth_learning_match and not_suitable_match:
            worth_text = worth_learning_match.group(1).strip()
            not_text = not_suitable_match.group(1).strip()
            if worth_text and not_text and worth_text == not_text:
                errors.append(
                    "书籍画像中'最值得学的地方'与'不适合直接参考的部分'内容相同，存在逻辑矛盾"
                )

        if content.count("。。") > 0:
            errors.append("书籍画像中发现重复句号语法错误")

    for name in METHOD_CARD_NAMES:
        method_path = project_dir / "outputs" / "方法卡" / f"{name}.md"
        if method_path.exists():
            content = read_utf8(method_path)

            if "## 为什么有效" not in content:
                errors.append(f"方法卡 '{name}' 缺少'为什么有效'章节")

            if re.search(r"- 关键 chunk：\s*$", content):
                errors.append(f"方法卡 '{name}' 证据锚点未填写具体chunk")

            if re.search(r"- 写法 \d+：\s*（至少 \d+ 个", content):
                errors.append(f"方法卡 '{name}' 存在未填写的可复用写法槽位（仍为模板提示文字）")
    ...
    if case_cards_dir.exists():
        for path in sorted(case_cards_dir.glob("*.md")):
            ...
            if "## 为什么有效" not in content:
                errors.append(f"案例卡 '{path.stem}' 缺少'为什么有效'章节")

            if "## 不可照搬" not in content:
                errors.append(f"案例卡 '{path.stem}' 缺少'不可照搬'章节")

            if "## 可复用做法" not in content:
                errors.append(f"案例卡 '{path.stem}' 缺少'可复用做法'章节")

    return errors



def extract_stage_from_case_card(content: str) -> str | None:
    """从案例卡内容中提取阶段信息"""
    # 在"最适合在什么创作阶段参考"字段中查找阶段关键词
    stage_patterns = [
        r"最适合在什么创作阶段参考[：:]\s*(.*?)(?:\n|$)",
        r"发生时机[：:]\s*(.*?)(?:\n|$)",
        r"阶段[：:]\s*(.*?)(?:\n|$)",
    ...
    for pattern in stage_patterns:
        ...
        if match:
            ...
            for stage in NARRATIVE_STAGE_NAMES:
...
def check_case_coverage(project_dir: Path, summary: dict) -> tuple[list[str], list[str]]:
    """检查案例卡是否覆盖小说各阶段，并验证各阶段数量要求"""
    warnings = []
    suggestions = []
    
    if summary["ready_case_cards"] == 0:
        warnings.append("没有正式案例卡，无法检查覆盖度")
    ...
    # 统计各阶段的案例卡数量
    stage_requirements = dict(NARRATIVE_STAGE_REQUIREMENTS)
    stage_counts: dict[str, int] = {stage: 0 for stage in stage_requirements}
    ...
    if case_cards_dir.exists():
        for path in sorted(case_cards_dir.glob("*.md")):
            content = read_utf8(path)
            template_content = build_case_card_template()
            ...
            # 提取阶段信息
            stage = extract_stage_from_case_card(content)
            if stage and stage in stage_counts:
                stage_counts[stage] += 1
            else:
                uncategorized_cases += 1
    
    # 检查各阶段是否满足最少数量要求
    total_required = sum(stage_requirements.values())
    total_actual = sum(stage_counts.values())
    ...
    for stage, required in stage_requirements.items():
        ...
        if actual < required:
            underflow_stages.append(f"{stage}({actual}/{required})")
    ...
    # 生成警告和建议
    if underflow_stages:
        req_str = ", ".join(f"{k}≥{v}" for k, v in stage_requirements.items())
        warnings.append(
            f"以下阶段案例卡数量未达标：{', '.join(underflow_stages)}。"
            f"要求：{req_str}"
        )
    
    # 总体数量检查
    if total_actual < total_required:
        req_detail = "+".join(f"{k}{v}" for k, v in stage_requirements.items())
        suggestions.append(
            f"当前正式案例卡共 {total_actual} 张，建议至少 {total_required} 张 "
            f"({req_detail})"
        )
    elif uncategorized_cases > 0:
        suggestions.append(
            f"有 {uncategorized_cases} 张案例卡未明确标注阶段，"
            f"建议在'最适合在什么创作阶段参考'字段中注明阶段"
        )
    
    # 显示各阶段分布情况
    stage_summary = ", ".join([f"{stage}:{count}" for stage, count in stage_counts.items()])
    suggestions.append(f"案例卡阶段分布：{stage_summary}")
    
    return warnings, suggestions


def check_output_diversity(project_dir: Path, summary: dict) -> tuple[list[str], list[str]]:
    """检查输出产物的多样性"""
    warnings = []
    suggestions = []
    ...
    case_cards_dir = project_dir / "outputs" / "案例卡"
    if case_cards_dir.exists():
        case_files = list(case_cards_dir.glob("*.md"))
        if case_files:
            # 分析案例卡文件名中的类型信息
            case_types = set()
            for path in case_files:
                # 从文件名提取类型信息（假设格式：NN-类型-描述.md）
                name_parts = path.stem.split("-")
                if len(name_parts) >= 2:
                    case_types.add(name_parts[1])
            
            if len(case_types) < 2 and len(case_files) >= 3:
                suggestions.append(f"案例卡类型较少（{len(case_types)} 种），建议增加不同类型的案例")
    
    # 检查方法卡内容的多样性
    method_variety_score = 0
    for name in METHOD_CARD_NAMES:
        method_path = project_dir / "outputs" / "方法卡" / f"{name}.md"
        if method_path.exists():
            content = read_utf8(method_path)
            ...
            if len(content) > 1000:  # 内容较长
                method_variety_score += 1
    
    if method_variety_score < 3:
        suggestions.append("部分方法卡内容可能较简略，建议丰富方法描述和例子")
    
    return warnings, suggestions


def check_output_completeness(project_dir: Path, summary: dict) -> tuple[list[str], list[str]]:
    """检查输出产物的完整性"""
    warnings = []
    suggestions = []
    ...
    if book_profile_path.exists():
        ...
        if missing_sections:
            warnings.append(f"书籍画像缺少以下部分：{', '.join(missing_sections)}")
    ...
    for name in METHOD_CARD_NAMES:
        ...
        if method_path.exists():
            content = read_utf8(method_path)
            template_content = method_card_defaults.get(name, "")
            if template_content and content == template_content:
                warnings.append(f"方法卡 '{name}' 仍然是模板内容，未进行实际填写")
    ...
    if case_cards_dir.exists():
        for path in sorted(case_cards_dir.glob("*.md")):
            ...
            if content == template_content:
                warnings.append(f"案例卡 '{path.stem}' 仍然是模板内容，未进行实际填写")
            elif len(content) < 500:  # 内容过短
                suggestions.append(f"案例卡 '{path.stem}' 内容可能较简略，建议丰富分析和结论")
    
    return warnings, suggestions


def build_check_report(project_dir: Path, finalize_mode: bool, deep_check: bool = False) -> dict:
    ...
    # 深度质量检查（在finalize模式或deep_check模式下进行）
    deep_quality_issues = []
    deep_quality_warnings = []
    deep_suggestions = []
    
    if finalize_mode or deep_check or (summary["processed_chunk_count"] > 0 and summary["ready_case_cards"] > 0):
        # 只在有实际内容时进行深度检查
        deep_results = perform_deep_quality_checks(project_dir, summary)
        deep_quality_issues.extend(deep_results.get("issues", []))
        deep_quality_warnings.extend(deep_results.get("warnings", []))
        deep_suggestions.extend(deep_results.get("suggestions", []))

    # 在finalize模式下，深度质量问题转为阻塞项
    if finalize_mode:
        blocking_issues.extend(deep_quality_issues)
        warnings.extend(deep_quality_warnings)
    else:
        # 在普通check模式下，深度问题都作为警告
        warnings.extend(deep_quality_issues)
        warnings.extend(deep_quality_warnings)
    ...
    return {
        ...
        "warnings": warnings,
        "deep_suggestions": deep_suggestions,
        "deep_quality_issues": deep_quality_issues if not finalize_mode else [],
        "deep_quality_warnings": deep_quality_warnings,
    }
...
def print_check_summary(report: dict, finalize_mode: bool) -> None:
    ...
    # 显示深度质量检查的改进建议
    if report.get("deep_suggestions"):
        print("\n改进建议（深度检查）:")
        for item in report["deep_suggestions"]:
            print(f"- {item}")
    
    # 在非finalize模式下显示深度质量问题
    if not finalize_mode and report.get("deep_quality_issues"):
        print("\n深度质量问题（在finalize模式下将成为阻塞项）:")
        for item in report["deep_quality_issues"]:
            print(f"- {item}")
...
def sync_support_files(project_dir: Path, force: bool) -> list[str]:
    ...
    def sync_skeleton_only(relative_path: str, content: str) -> None:
        """只同步骨架文件，绝不覆盖用户已编辑的产出文件"""
        path = project_dir / relative_path
        if path.exists() and is_modified(path, content):
            return
        write_text(path, content, force)
        touched.append(relative_path)
    ...
    sync_file("workspace/stage-plan.md", render_stage_plan(chunks, stage_size, stage_mode))
    sync_file("workspace/use-guide.md", render_use_guide(book_name, len(chunks), stage_mode))
    sync_file("workspace/案例卡模板.md", build_case_card_template())
    sync_file("workspace/推荐案例类型.md", build_case_type_reference())

    sync_skeleton_only("drafts/书籍画像草稿.md", build_book_profile_draft(book_name))
    sync_skeleton_only("drafts/方法候选笔记.md", build_method_candidate_notes())
    sync_skeleton_only("drafts/案例候选笔记.md", build_case_candidate_notes())

    sync_skeleton_only("outputs/书籍画像.md", build_book_profile_output())
    sync_skeleton_only("outputs/方法卡/开篇规划方法卡.md", build_opening_method_card())
    sync_skeleton_only("outputs/方法卡/大纲设计方法卡.md", build_outline_method_card())
    sync_skeleton_only("outputs/方法卡/章节类型与写作模式方法卡.md", build_chapter_method_card())
    sync_skeleton_only("outputs/方法卡/冲突与角色设计方法卡.md", build_conflict_character_method_card())
    sync_skeleton_only("outputs/方法卡/读者情绪管理方法卡.md", build_emotion_method_card())
    sync_skeleton_only("outputs/方法卡/长篇创作与文风优化方法卡.md", build_longform_style_method_card())

    for chunk in chunks:
        sync_skeleton_only(f"extraction-cards/{chunk.chunk_id}.md", render_extraction_card_template(chunk))
...
def handle_init(args: argparse.Namespace) -> int:
    ...
    progress["stage_mode"] = args.stage_mode
...
def handle_check(args: argparse.Namespace) -> int:
    project_dir = Path(args.project_path).expanduser().resolve()
    report = build_check_report(project_dir, finalize_mode=False, deep_check=args.deep)
...
def handle_finalize(args: argparse.Namespace) -> int:
    project_dir = Path(args.project_path).expanduser().resolve()
    report = build_check_report(project_dir, finalize_mode=True, deep_check=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_parser.add_argument("--stage-size", type=int, default=8, help="每多少块做一次阶段收束，默认 8（仅在 fixed 模式下使用）")
    init_parser.add_argument("--stage-mode", choices=["narrative", "fixed"], default="narrative", help="阶段划分模式：narrative（叙事阶段，默认）或 fixed（固定数量均分）")
    ...
    check_parser.add_argument("--deep", action="store_true", help="执行深度质量检查（包括逻辑错误、格式错误、语法错误检查）")
